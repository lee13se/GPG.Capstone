...
if (strlen($so)>0)
{
	...
	while($row = mysql_fetch_array($results))
	{
		...
		if ($hint=="")
        {
			$hint = '<a href="/enterShipments.php?salesOrderNum='.$SalesOrderNum.'">'.$SalesOrderNum.'</a>';
		}
	
		//$hint is not blank, so append the next result
		else
        {
			$hint=$hint .'<br/>'. '<a href="/enterShipments.php?salesOrderNum='.$SalesOrderNum.'">'.$SalesOrderNum.'</a>';
		}
