...
if (strlen($q)>0)
{
	...
	$results = mysql_query("SELECT 
					   		OldPartNum, PartNum, RawMaterialID 
					   FROM
					   		Part_Info 
					   WHERE
					   		OldPartNum LIKE '".$q."%'
	...
	while($row = mysql_fetch_array($results))
	{
		...
		if ($hint=="")
        {
			$hint = '<a href="/PartView.php?PartNum='.$PartNum.'&RawMaterialID='.$RawMaterialID.'">'.$OldPartNum.'</a>';
		}
	
		//$hint is not blank, so append the next result
		else
        {
			$hint=$hint .'<br/>'. '<a href="/PartView.php?PartNum='.$PartNum.'&RawMaterialID='.$RawMaterialID.'">'.$OldPartNum.'</a>';
		}
