...
if (strlen($q)>0)
{
	...
	while($row = mysql_fetch_array($results))
	{
		...
		if ($hint=="")
        {
			$hint = '<a href="/ViewClient.php?clientNum='.$ClientNum.'">'.$OldCompanyID.'</a>';
		}
	
		//$hint is not blank, so append the next result
		else
        {
			$hint=$hint .'<br/>'. '<a href="/ViewClient.php?clientNum='.$ClientNum.'">'.$OldCompanyID.'</a>';
		}
