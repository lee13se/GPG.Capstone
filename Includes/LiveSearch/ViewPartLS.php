...
if (strlen($q)>0)
{
	...
	while($row = mysql_fetch_array($results))
	{
		...
		if ($hint=="")
        {
			$hint = '<a href="/ViewPart.php?clientNum='.$clientNum.'&partNum='.$PartNum.'">'.$OldPartNum.'</a>';
		}
	
		//$hint is not blank, so append the next result
		else
        {
			$hint=$hint .'<br/>'. '<a href="/ViewPart.php?clientNum='.$clientNum.'&partNum='.$PartNum.'">'.$OldPartNum.'</a>';
		}
